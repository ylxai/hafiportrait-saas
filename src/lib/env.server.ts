...
const serverEnvSchema = publicEnvSchema.extend({
  ...
  // Required: every environment that runs background jobs must point at a
  // deletion-worker endpoint. Validating at startup surfaces missing config
  // immediately instead of failing silently when an enqueue call fires.
  CLOUDFLARE_WORKER_URL: z.string().url('CLOUDFLARE_WORKER_URL must be a valid URL'),
});
