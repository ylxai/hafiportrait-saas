...
/**
 * Cross-gallery dedup support (PR #76 / issue #10).
 *
...
export async function collectPhotoDeletionPayloads(
  ...
  return photos
    .map<PhotoDeletionPayload>((photo) => {
      ...
      // If the r2Key is still referenced by another Photo row, drop the
      // storage-side fields so the worker has nothing to do for this row.
      // The accompanying `thumbnailUrl` / Cloudinary `publicId` follow
      // the same lifecycle when produced via dedup, so they ride along.
      const isShared =
