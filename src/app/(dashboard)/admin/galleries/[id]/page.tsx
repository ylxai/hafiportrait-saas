...
export default function GalleryDetailPage() {
  ...
  const {
    ...
    galleryId
      ? `/api/admin/galleries/${galleryId}/photos?page=${currentPage}&limit=${photosPerPage}`
      : null,
    fetcher,
  );
  ...
  const selectedPhotoIdsFromServer =
    latestSelection?.photos.map((p) => p.photoId) || [];
    ...
    <div>
      ...
      {showSelectionView && (
        <div className="mb-6 bg-muted border border-border rounded-xl p-4">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4">
            ...
            >
              {gallery.isSelectionLocked
                ? "🔓 Buka Kunci (Klien bisa memilih lagi)"
                : "🔒 Kunci Seleksi (Klien tidak bisa memilih)"}
            </button>
          </div>
          {gallery.isSelectionLocked ? (
            <div>
              <p className="text-sm text-muted-foreground mb-3">
                Client telah memilih{" "}
                <span className="font-bold text-primary">
                  {selectedPhotoIdsFromServer.length}
                </span>{" "}
                foto
            ...
            <div className="flex flex-col gap-2">
              <p className="text-sm text-muted-foreground">
                Status seleksi saat ini sedang{" "}
                <strong className="text-primary">Terbuka</strong>. Klien masih
                dapat mengubah dan mengirimkan pilihan foto.
              </p>
              {selectedPhotoIdsFromServer.length > 0 && (
                <p className="text-sm font-medium text-primary">
                  Pilihan terakhir klien: {selectedPhotoIdsFromServer.length}{" "}
                  foto.
                </p>
              )}
            </div>
            ...
            {totalPhotos > 0 && (
              <>
                <button
                  ...
                  className={`px-3 py-2 sm:py-1 text-sm rounded-lg transition-smooth cursor-pointer border border-border ${
                    bulkMode
                      ...
                      : "bg-card text-foreground hover:bg-primary/20 hover:text-primary hover:border-primary/50"
                  }`}
                >
                ...
                {bulkMode && (
                  ...
                  >
                    {selectedPhotoIdsForBulk.size === photos.length
                      ? "Batal Semua"
                      : "Pilih Semua"}
                  </button>
                )}
                <button
                  ...
                  className={`px-3 py-2 sm:py-1 text-sm rounded-lg transition-smooth cursor-pointer border border-border ${
                    reorderMode
                      ...
                      : "bg-card text-foreground hover:bg-accent/20 hover:text-accent-foreground hover:border-accent/50"
                  }`}
                >
        ...
        {photos.length === 0 ? (
          <div className="text-center py-8 sm:py-12 border-2 border-dashed border-border rounded-lg">
            <p className="text-muted-foreground">
              Belum ada foto. Upload foto untuk gallery ini.
            </p>
          </div>
        ) : (
          <>
            ...
            {totalPages > 1 && (
              <div className="mt-8 flex flex-col sm:flex-row items-center justify-between gap-4 border-t border-border pt-4">
                <div className="text-sm text-muted-foreground">
                  Menampilkan {(currentPage - 1) * photosPerPage + 1} -{" "}
                  {Math.min(currentPage * photosPerPage, totalPhotos)} dari{" "}
                  {totalPhotos} foto
                </div>
      ...
      <div className="bg-card/50 backdrop-blur-xl border border-border shadow-2xl rounded-3xl p-4 sm:p-6">
        ...
        <div className="mt-4 flex items-center gap-3">
          ...
          {settingsMessage && (
            <span
              className={`text-sm ${settingsMessage.includes("success") ? "text-primary" : "text-destructive"}`}
            >
              {settingsMessage}
            </span>
          )}
