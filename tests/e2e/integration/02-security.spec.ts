import { test, expect } from "@playwright/test";

test.describe("Security and Authorization", () => {
  test("should block unauthenticated access to admin routes", async ({
    page,
  }) => {
    const adminRoutes = [
      "/admin",
      "/admin/galleries",
      "/admin/events",
      "/admin/clients",
      "/admin/packages",
      "/admin/storage",
      "/admin/finance",
      "/admin/analytics",
    ];

    for (const route of adminRoutes) {
      await page.goto(route);
      await page.waitForURL("/login");
      await expect(page).toHaveURL(/\/login/);
    }
  });

  test("should block unauthenticated API requests", async ({ page }) => {
    const response = await page.request.get("/api/admin/galleries");
    expect(response.status()).toBe(401);
  });

  test("should allow public gallery access without auth", async ({ page }) => {
    await page.goto("/gallery/test-token-123");

    // Should not redirect to login
    await expect(page).toHaveURL(/\/gallery\//);
  });

  test("should allow public booking access without auth", async ({ page }) => {
    await page.goto("/booking");

    await expect(page).toHaveURL(/\/booking/);
    await expect(page).not.toHaveURL(/\/login/);
  });

  test("should validate gallery token", async ({ page }) => {
    await page.goto("/gallery/invalid-token-xyz");

    await expect(page.locator("text=Gallery not found")).toBeVisible();
  });

  test("should prevent XSS in search input", async ({ page, context }) => {
    await context.addInitScript(() => {
      (window as { xssTriggered?: boolean }).xssTriggered = false;
      window.alert = () => {
        (window as { xssTriggered?: boolean }).xssTriggered = true;
      };
    });

    await page.goto("/admin");
    await page.fill(
      '[data-testid="global-search"]',
      '<script>alert("XSS")</script>',
    );
    await page.keyboard.press("Enter");

    const xssTriggered = await page.evaluate(
      () => (window as { xssTriggered?: boolean }).xssTriggered,
    );
    expect(xssTriggered).toBe(false);
  });

  test("should sanitize user input in forms", async ({ page }) => {
    await page.goto("/admin/clients");
    await page.click("text=Add Client");

    await page.fill('input[name="nama"]', '<script>alert("XSS")</script>');
    await page.fill('input[name="email"]', "test@test.com");
    await page.click('button[type="submit"]');

    // Should display escaped text, not execute script
    await expect(page.locator("text=<script>")).toBeVisible();
  });

  test("should enforce HTTPS in production", async ({ page }) => {
    // Skip in local dev
    if (process.env.NODE_ENV !== "production") {
      test.skip();
    }

    await page.goto("/admin");
    await expect(page).toHaveURL(/^https:/);
  });

  test("should prevent email enumeration on magic link", async ({ page }) => {
    // Test with non-existent email
    const response1 = await page.request.post("/api/portal/auth/magic-link", {
      data: { email: "nonexistent@example.com" },
    });

    // Test with existing email
    const response2 = await page.request.post("/api/portal/auth/magic-link", {
      data: { email: "existing@example.com" },
    });

    // Both should return same success message to prevent enumeration
    expect(response1.status()).toBe(200);
    expect(response2.status()).toBe(200);

    const data1 = await response1.json();
    const data2 = await response2.json();

    expect(data1.success).toBe(true);
    expect(data2.success).toBe(true);
  });

  test("should handle Prisma P2025 error gracefully", async ({ page }) => {
    // Try to access non-existent gallery
    const response = await page.request.get(
      "/api/public/gallery/clxxxxxxxxxxxxxxxxxxx",
    );

    expect(response.status()).toBe(404);
    const data = await response.json();
    expect(data.success).toBe(false);
    expect(data.error).toBe("Gallery not found");

    // Should not expose internal error codes
    expect(data.error).not.toContain("P2025");
  });

  test("should validate photoIds array in selection", async ({ page }) => {
    const response = await page.request.post(
      "/api/public/gallery/test-token/submit",
      {
        data: { photoIds: "not-an-array" },
      },
    );

    expect(response.status()).toBe(400);
    const data = await response.json();
    expect(data.success).toBe(false);
    expect(data.error).toContain("photoIds");
  });

  test("should validate photoIds are valid CUIDs", async ({ page }) => {
    const response = await page.request.post(
      "/api/public/gallery/test-token/submit",
      {
        data: { photoIds: ["invalid-id", "another-invalid"] },
      },
    );

    expect(response.status()).toBe(400);
    const data = await response.json();
    expect(data.success).toBe(false);
  });

  test("should prevent SQL injection in gallery token", async ({ page }) => {
    const maliciousToken = "'; DROP TABLE galleries; --";
    const response = await page.request.get(
      `/api/public/gallery/${encodeURIComponent(maliciousToken)}`,
    );

    expect(response.status()).toBe(400);
    const data = await response.json();
    expect(data.error).toContain("Invalid gallery token format");
  });

  test("should prevent NoSQL injection in search", async ({ page }) => {
    const maliciousQuery = { $ne: null };
    const response = await page.request.post("/api/admin/search", {
      data: { query: maliciousQuery },
    });

    // Should handle gracefully, not execute injection
    expect([400, 401]).toContain(response.status());
  });

  test("should validate CUID format for gallery tokens", async ({ page }) => {
    const invalidTokens = [
      "short",
      "contains spaces",
      "../../../etc/passwd",
      "<script>alert(1)</script>",
    ];

    for (const token of invalidTokens) {
      const response = await page.request.get(
        `/api/public/gallery/${encodeURIComponent(token)}`,
      );
      expect(response.status()).toBe(400);
      const data = await response.json();
      expect(data.error).toContain("Invalid gallery token format");
    }
  });

  test("should sanitize error messages", async ({ page }) => {
    // Try to trigger an error with sensitive data
    const response = await page.request.post("/api/public/booking", {
      data: {
        nama: "Test",
        email: "test@example.com",
        phone: "+628123456789",
        eventDate: "invalid-date",
      },
    });

    expect(response.status()).toBe(400);
    const data = await response.json();

    // Error should not contain database schema info or internal paths
    expect(data.error).not.toContain("prisma");
    expect(data.error).not.toContain("/home/");
    expect(data.error).not.toContain("DATABASE_URL");
  });
});
